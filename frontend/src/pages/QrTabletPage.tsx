...
export default function QrTabletPage() {
  ...
  const handleLocationScan = useCallback(
    ...
    [assignMutation, lotNumber, preview, quantity, unitId],
  );

  const handleDecoded = useCallback(
    (scanned: string) => {
      const toLocation = scanMode === "location" || (scanMode === "idle" && !!preview);
      if (toLocation) {
        if (!preview) {
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col items-center justify-center bg-stripe-canvas-soft px-6 py-10">
      <div className="w-full max-w-xl rounded-2xl border border-stripe-hairline bg-white p-8 shadow-sm">
        <h1 className="mb-2 text-center text-3xl font-extrabold text-brand-dark">
          Quét QR nhập kho
        </h1>
