...
export default function QrTabletOutboundDetailPage() {
  ...
  const handleDetailRowExpand = useCallback(
    (expanded: boolean) => {
      if (expanded) void refetchDetails();
    },
    [refetchDetails],
  );
  ...
  const hasPreCompletedAllocation = useMemo(
    ...
    [allocationTasks, details, useManualAllocationFlow],
  );
  ...
  const robotTaskColumns: ColumnsType<OutboundRobotTask> = [
    ...
    },
    {
      ...
      dataIndex: "status",
      key: "status",
      width: isManualOutbound || useManualAllocationFlow ? 220 : 150,
      render: (_, record) => {
            ...
            {activeTab === "list" ? (
              ...
              <Table
                ...
                expandable={{
                  ...
                  onExpand: handleRobotTaskRowExpand,
                }}
              />
            )}
          </div>
