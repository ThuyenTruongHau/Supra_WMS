...
export default function WorkerInboundConfirmPage() {
  ...
  const { vehicleNumber, detailId } = useParams<{
  ...
  return (
    <div className="min-h-[100dvh] bg-stripe-canvas-soft flex flex-col items-center justify-center p-6 text-center">
      <h1 className="text-2xl font-extrabold text-stripe-ink mb-2">Xác nhận cất hàng</h1>
      ...
      <Button
        ...
        onClick={() =>
          navigate(
            `/worker/vehicles/${vehicleNumber || ""}/skus/${detailId || ""}/pickup`,
          )
        }
      >
        Quay lại
